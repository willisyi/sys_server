...
static void RestartAll()
{
	...
	//system("killall -2 dvr_rdk_demo_link_api.out\n");
	...
	tConfig.nFrameRate1 = pSysInfo->lan_config.frameRateVal1/1000;
...
static void *PollingThread(void *arg){
	...
	__u8 bIsUsbModify = 0;
	int nExtFunc = 0;
//	int	dev_ver = Check_device_version(); //commented by lyy
	int	dev_ver = 1;
...
//	extern int Get_AlarmDuration(void);
	...
	while(1){
		if(gPlatform == 1){
//			dhcp = dm355_gio_read(GIO_DHCP);  // commented by lyy
                        dhcp = 0;
			if(!dhcp)
				dhcp_fail = 0;
			if(sdcard_en)
			...
			if( dev_ver == 1 )
			{
//				isAlarmReSet=dm355_gio_read(GIO_ALARMRESET); //commented by lyy
				isAlarmReSet = 0;
			...
			if (link_status!=-1){
				if(GIO_now==link_status){}
					...
					GIO_now=link_status;
			//		dm355_gio_write(GIO_LED_3,!(GIO_now));  //commented by lyy
				}
			}
			...
			if(durationTimer > 0){
				...
				Sub_AlarmDuration();
//				dm355_gio_write(GIO_ALARMOUT,!(pSysInfo->lan_config.gioouttype)); //commented by lyy
			}
			if (!gResetbttn_Error)
			//	isReset = dm355_gio_read(GIO_RESET_TO_DEFAULT) ? 0 : 1; //commented by lyy
                           isReset = 0;
		}
...
int SystemInit()
{
	...
	StreamEnv_t tConfig;
	SysInfo *pSysInfo;
//	gResetbttn_Error = dm355_gio_read(GIO_RESET_TO_DEFAULT) ? 0 : 1;  //commented by lyy
    gResetbttn_Error = 0;
	//gPlatform = Platform_check();  //commented by lyy
    gPlatform = 1;
	if(gPlatform == -1){
	...
	tConfig.nBitrate1 = pSysInfo->lan_config.nMpeg41bitrate;
	tConfig.nBitrate2 = pSysInfo->lan_config.nMpeg42bitrate;
	tConfig.nVideocodeclevel = pSysInfo->lan_config.nVideocodeclevel;
	tConfig.audioenable = pSysInfo->audio_config.audioenable;
	tConfig.audiocodectype = pSysInfo->audio_config.codectype;
	tConfig.audiobitrate = pSysInfo->audio_config.bitrate;
	tConfig.audioinvolume = pSysInfo->audio_config.audioinvolume;
	...
	tConfig.audioalarm = pSysInfo->audio_config.alarmON;
	tConfig.audioalarmlevel = pSysInfo->audio_config.alarmlevel;
	StartStream(&tConfig); //commented by lyy
/* commented by lyy */
#if 0
