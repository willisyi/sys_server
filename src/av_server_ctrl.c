...
const int audio_bitrate_g726[2][4] = {{16000,24000,32000,40000},{32000,48000,64000,80000}};
...
int StartStream(StreamEnv_t* pConfig)
{
	...
	char *audio_codec[4] = {"AUDIO G711","AUDIO G726","AUDIO G729","ADUIO AAC"};
	...
	char ps_packet[] = "LocalStorage";
        char rtsp_enable[] = "RTSP";
	char *video_codec_level[3] = {"HIGH", "MAIN", "BASE"};
	char *chip_cfg[2] = {"DM8168","DM365"};  //commented by lyy
	char *sw_brc1, *sw_brc2, *sw_mir, *sw_aewb;
	char *sw_mecfg1, *sw_mecfg2, *sw_mecfg3;
//	int chipConfig = CheckCpuSpeed();  commented by lyy
//      int chipConfig = 1;
...
*/
	...
	if(pConfig -> audioenable) {
		...
		if(pConfig -> audiocodectype == 0)
			sprintf(aud, " G711 uLaw");
		else if (pConfig -> audiocodectype == 1)
			sprintf(aud, " G711 aLaw");
		else if (pConfig -> audiocodectype == 2)
			sprintf(aud, " G726 %d",audio_bitrate_g726[pConfig -> audioSampleRate][pConfig -> audiobitrate]);
		else if (pConfig -> audiocodectype == 3)
			sprintf(aud, " G729 ");
		else if (pConfig -> audiocodectype == 4)
			sprintf(aud, "AAC %d",audio_bitrate_aac[pConfig -> audioSampleRate][pConfig -> audiobitrate]);
		attach_opt(cmdopt, aud);
...
/*
*Videocodecmode: 0 single H264 
*               1 single mpeg4
*nVideocodecres: 0 1080P(1920*1080)
*              1 1080I(1920*1080)
*              2 720P(1280*720)
*              3 720I(1280*720)
*              4 480P(720*480)
...
*/
    switch (pConfig -> nVideocodecmode) 
    {
		...
		{   //single mpeg4
		    if (pConfig -> nVideocodecres == 5)
		    {
		    	sprintf(cmd, "./dvr_rdk_demo_link_api.out %s 480I%d MPEG4 %d %s &\n",cmdopt,pConfig->nFrameRate1,pConfig -> nBitrate1, sw_brc1);
				fSetVideoSize(2, 720, 480);
				fSetStreamConfig(0, 720, 480, codec_tpye[1], MPEG4_1_PORTNUM);
		    }
		    else if (pConfig -> nVideocodecres == 4)
		    {
		        sprintf(cmd, "./dvr_rdk_demo_link_api.out %s 480P%d MPEG4 %d %s &\n",cmdopt,pConfig->nFrameRate1, pConfig -> nBitrate1, sw_brc1);
				fSetVideoSize(2, 720, 480);
				fSetStreamConfig(0, 720, 480, codec_tpye[1], MPEG4_1_PORTNUM);
		    }
			else if (pConfig -> nVideocodecres == 3)
		    {
		        sprintf(cmd, "./dvr_rdk_demo_link_api.out %s 720I%d MPEG4 %d %s &\n",cmdopt,pConfig->nFrameRate1, pConfig -> nBitrate1, sw_brc1);
				fSetVideoSize(2, 1280, 720);
				fSetStreamConfig(0, 1280, 720, codec_tpye[1], MPEG4_1_PORTNUM);
		    }
			else if (pConfig -> nVideocodecres == 2)
			{
		        sprintf(cmd, "./dvr_rdk_demo_link_api.out %s 720P%d MPEG4 %d %s &\n",cmdopt,pConfig->nFrameRate1, pConfig -> nBitrate1, sw_brc1);
				fSetVideoSize(2, 1280, 720);
				fSetStreamConfig(0, 1280, 720, codec_tpye[1], MPEG4_1_PORTNUM);
		    }
			else if (pConfig -> nVideocodecres == 1)
			{
		        sprintf(cmd, "./dvr_rdk_demo_link_api.out %s 1080I%d MPEG4 %d %s &\n",cmdopt,pConfig->nFrameRate1, pConfig -> nBitrate1, sw_brc1);
				fSetVideoSize(2, 1920, 1080);
				fSetStreamConfig(0, 1920, 1080, codec_tpye[1], MPEG4_1_PORTNUM);
		    }
			else if (pConfig -> nVideocodecres == 0)
			{
		        sprintf(cmd, "./dvr_rdk_demo_link_api.out %s 1080P%d MPEG4 %d %s &\n",cmdopt, pConfig->nFrameRate1,pConfig -> nBitrate1, sw_brc1);
				fSetVideoSize(2, 1920, 1080);
				fSetStreamConfig(0, 1920, 1080, codec_tpye[1], MPEG4_1_PORTNUM);
		    }
			break;
		...
		{  //single H264
			...
			if (pConfig -> nVideocodecres == 5)
			{
				sprintf(cmd, "./dvr_rdk_demo_link_api.out %s 480I%d H264 %d %s %s &\n",
					cmdopt, pConfig->nFrameRate1, pConfig -> nBitrate1, sw_brc1, video_codec_level[pConfig->nVideocodeclevel]);
				fSetVideoSize(5, 720, 480);
				fSetStreamConfig(0, 720, 480, codec_tpye[0], H264_1_PORTNUM);
			}
			if (pConfig -> nVideocodecres == 4)
			{
				sprintf(cmd, "./dvr_rdk_demo_link_api.out %s 480P%d H264 %d %s %s &\n",
					cmdopt, pConfig->nFrameRate1, pConfig -> nBitrate1, sw_brc1, video_codec_level[pConfig->nVideocodeclevel]);
				fSetVideoSize(5, 720, 480);
				fSetStreamConfig(0, 720, 480, codec_tpye[0], H264_1_PORTNUM);
			}
			if (pConfig -> nVideocodecres == 3)
			{
				sprintf(cmd, "./dvr_rdk_demo_link_api.out %s 720I%d H264 %d %s %s &\n",
					cmdopt, pConfig->nFrameRate1, pConfig -> nBitrate1, sw_brc1, video_codec_level[pConfig->nVideocodeclevel]);
				fSetVideoSize(5, 1280, 720);
				fSetStreamConfig(0, 1280, 720, codec_tpye[0], H264_1_PORTNUM);
			}
			if (pConfig -> nVideocodecres == 2)
			{
				sprintf(cmd, "./dvr_rdk_demo_link_api.out %s 720P%d H264 %d %s %s &\n",
					cmdopt, pConfig->nFrameRate1, pConfig -> nBitrate1, sw_brc1, video_codec_level[pConfig->nVideocodeclevel]);
				fSetVideoSize(5, 1280, 720);
				fSetStreamConfig(0, 1280, 720, codec_tpye[0], H264_1_PORTNUM);
			}
			...
			{
				sprintf(cmd, "./dvr_rdk_demo_link_api.out %s 1080I%d H264 %d %s %s &\n",
					cmdopt, pConfig->nFrameRate1, pConfig -> nBitrate1, sw_brc1, video_codec_level[pConfig->nVideocodeclevel]);
				fSetVideoSize(5, 1920, 1080);
				fSetStreamConfig(0, 1920, 1080, codec_tpye[0], H264_1_PORTNUM);
			}
			if (pConfig -> nVideocodecres == 0)
			{
				sprintf(cmd, "./dvr_rdk_demo_link_api.out %s 1080P%d H264 %d %s %s &\n",
					cmdopt, pConfig->nFrameRate1, pConfig -> nBitrate1, sw_brc1,video_codec_level[pConfig->nVideocodeclevel]);
				fSetVideoSize(5, 1920, 1080);
				fSetStreamConfig(0, 1920, 1080, codec_tpye[0], H264_1_PORTNUM);
			}
...
int SetFramerate1(unsigned char value)
{
	...
	if(ret == 0) {
		...
		//	frameRateVal = pSysInfo->lan_config.chipConfig ? enc_framerate_night_1_dm368[i][j][value]:enc_framerate_night_1_dm365[i][j][value];
		  { //      dbg("[sys_server] framterate1 night value is %d\n",value);
		       frameRateVal = enc_framerate_night_1_dm8168[i][j][value];
		       	  dbg("[sys_server] framterate1 night val is %d\n", frameRateVal);  
			}
		else{
		//	frameRateVal = pSysInfo->lan_config.chipConfig ? enc_framerate_day_1_dm368[i][j][value]:enc_framerate_day_1_dm365[i][j][value];
		  //   dbg("[sys_server] framterate1 day value is %d\n",value);
		       frameRateVal = enc_framerate_day_1_dm8168[i][j][value];
		       dbg("[sys_server] framterate1 day val is %d\n", frameRateVal); 
			}    
		ret = fSetFramerateVal1(frameRateVal);
		dbg("[sys_server] success is %d\n",ret);
		if(!IsRestart())
